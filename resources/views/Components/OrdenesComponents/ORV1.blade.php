<div class="col-xl-4 col-lg-5">
  <div style="position: sticky; top: 20px;">
    ...
    <div class="card shadow-sm border-0 rounded-3">
      ...
      <div class="card-body p-4">
        ...
        <div class="mb-3">
          ...
          <select class="form-select rounded-3 @error('IdArea') is-invalid @enderror" wire:model.live="IdArea">
        ...
        <div class="mb-3">
          ...
          <select class="form-select rounded-3 @error('IdLinea') is-invalid @enderror"
            ...
            @disabled(!$IdArea || $lineasDisponibles->isEmpty())>
            <option value="">
              {{ $IdArea && $lineasDisponibles->isNotEmpty() ? 'Seleccione línea' : 'Primero seleccione área' }}
            </option>
        ...
        {{-- Máquina --}}
        <div class="mb-3">
          <label class="form-label fw-semibold small">Máquina/Equipo <span class="text-danger">*</span></label>
          <select class="form-select rounded-3 @error('Maquina') is-invalid @enderror"
            wire:model.live="Maquina"
            @disabled(!$IdLinea || $maquinasDisponibles->isEmpty())>
            <option value="">
              {{ $IdLinea && $maquinasDisponibles->isNotEmpty() ? 'Seleccione máquina' : 'Primero seleccione línea' }}
            </option>
            @foreach($maquinasDisponibles as $m)
            <option value="{{ $m->Nombre }}">{{ $m->Nombre }}</option>
            @endforeach
          </select>
          @error('Maquina') <div class="invalid-feedback small">{{ $message }}</div> @enderror
        </div>

        {{-- Empleado --}}
        <div class="mb-3 position-relative" x-data="{ open: @entangle('mostrarDropdown') }" @click.outside="open = false">
          ...
          <template x-if="open">
            <ul class="list-group position-absolute w-100 shadow" style="z-index: 10;">
              ...
              <li class="list-group-item list-group-item-action"
                wire:click="seleccionarEmpleado('{{ $e['NumeroEmpleado'] }}','{{ $e['Nombre'] }}')"
        ...
        <button class="btn btn-primary w-100 rounded-pill py-2" wire:click="guardar">
    ...
    {{-- ============================================ --}}
    {{-- PRODUCCIÓN: Aprobar Arranque --}}
    {{-- ============================================ --}}
    @if(in_array($departamentoActual, ['Produccion', 'Calidad']))
    <div class="card shadow-sm border-0 rounded-3">
      <div class="card-header bg-white border-bottom px-4 py-3 rounded-top-3">
        <div class="d-flex justify-content-between align-items-center">
          <h5 class="mb-0 fw-semibold">
            <i class="fas fa-play-circle text-primary me-2"></i>Aprobar Arranque
          </h5>
      ...
      <div class="card-body p-4" style="max-height: 730px; overflow-y: auto;">
        ...
        <div class="alert alert-info border-0 rounded-3 mb-4">
          <div class="row g-2 small">
            ...
            <div class="col-12"><strong>Problema:</strong> {{ Str::limit($Descripcion, 100) }}</div>
          </div>
        </div>

        {{-- Hora de Cierre (bloqueada) --}}
        <div class="mb-3">
          <label class="form-label fw-semibold small">
            Hora de Cierre (Recepción Línea) <span class="text-danger">*</span>
          </label>
          <input type="datetime-local" class="form-control rounded-3 bg-light"
            wire:model="HoraRecepcionLinea"
            readonly disabled>
          <small class="text-muted">
            <i class="fas fa-lock me-1"></i>Asignado automáticamente de la hora de cierre
          </small>
        </div>
        ...
        <div class="alert alert-secondary border-0 rounded-3 mb-3 small">
          <div class="row">
            @if($tiempoEspera)
            <div class="col-6"><strong>Tiempo espera:</strong> {{ $tiempoEspera }}</div>
            @endif
            @if($tiempoArranque)
            <div class="col-6"><strong>Tiempo arranque:</strong> {{ $tiempoArranque }}</div>
            @endif
          </div>
        </div>
        @endif
        ...
        <div class="mb-3">
          <label class="form-label fw-semibold">Descripción del arranque <span class="text-danger">*</span></label>
          <textarea wire:model="DescripcionArranque" rows="4"
          ...
          @error('DescripcionArranque') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
        ...
        <div class="d-flex gap-2">
          ...
          <button class="btn btn-secondary rounded-pill py-2 px-4" wire:click="limpiarFormulario">
    ...
    {{-- ============================================ --}}
    {{-- MANTENIMIENTO: Cerrar Orden --}}
    {{-- ============================================ --}}
    @if(in_array($departamentoActual, ['Mantenimiento', 'IT']))
    <div class="card shadow-sm border-0 rounded-3">
      <div class="card-header bg-white border-bottom px-4 py-3 rounded-top-3">
        <div class="d-flex justify-content-between align-items-center">
          <h5 class="mb-0 fw-semibold">
            <i class="fas fa-check-circle text-success me-2"></i>Cerrar Orden
          </h5>
      ...
      <div class="card-body p-4" style="max-height: 730px; overflow-y: auto;">
        ...
        <div class="mb-3">
          ...
          <select class="form-select rounded-3 @error('Tipo') is-invalid @enderror" wire:model.live="Tipo">
    ...
    {{-- IT: Aprobar Arranque --}}
    {{-- ============================================ --}}
    @if($Status === 'cerrada' && !$HoraRecepcionLinea)
    {{-- Aprobar Arranque --}}
    <div class="card shadow-sm border-0 rounded-3">
      <div class="card-header bg-white border-bottom px-4 py-3 rounded-top-3">
        <div class="d-flex justify-content-between align-items-center">
      ...
      <div class="card-body p-4" style="max-height: 730px; overflow-y: auto;">
        ...
        <div class="mb-3">
          ...
          <input type="datetime-local" class="form-control rounded-3 bg-light"
            wire:model="HoraRecepcionLinea" readonly disabled>
          <small class="text-muted"><i class="fas fa-lock me-1"></i>Asignado automáticamente</small>
        </div>

        <div class="mb-3">
          <label class="form-label fw-semibold small">Hora Arranque <span class="text-danger">*</span></label>
          <input type="datetime-local" class="form-control rounded-3 @error('HoraArranque') is-invalid @enderror"
            wire:model="HoraArranque" min="{{ $HoraRecepcionLinea }}">
          @error('HoraArranque') <div class="invalid-feedback small">{{ $message }}</div> @enderror
        </div>
        ...
        <div class="mb-3">
          ...
          <textarea wire:model="DescripcionArranque" rows="4"
            class="form-control @error('DescripcionArranque') is-invalid @enderror"
            placeholder="Describe las observaciones del arranque..."></textarea>
          @error('DescripcionArranque') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
